...
import {
  useAssignPermissionToMerchantUserMutation,
  useGetMerchantPermissionsQuery,
  useGetMerchantRolePermissionsQuery,
  useRemovePermissionFromMerchantUserMutation,
...
export default function RoleDetailPage() {
  ...
  const { data: rolePermissionsData, isLoading: loadingRolePermissions } =
    useGetMerchantRolePermissionsQuery(rawId ?? "");
  const { mutateAsync: assignPermissions, isPending: assigning } =
    useAssignPermissionToMerchantUserMutation(rawId ?? "");
  const { mutateAsync: removePermissions, isPending: removing } =
    useRemovePermissionFromMerchantUserMutation(rawId ?? "");
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      if (toRemove.length > 0)
        await removePermissions({ rawId, permissionIds: toRemove });
      if (toAssign.length > 0)
        await assignPermissions({ rawId, permissionIds: toAssign });
