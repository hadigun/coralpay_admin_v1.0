...
export default function RoleDetailPage() {
  ...
  const { roleId: encodedRoleId } = useParams();
  const roleId = useMemo(() => {
    if (!encodedRoleId) return null;
    try {
      return decodeURIComponent(encodedRoleId as string);
    } catch {
      return encodedRoleId as string;
    }
  }, [encodedRoleId]);
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      if (toRemove.length > 0)
        await removePermissions({ roleId, permissionIds: toRemove });
      if (toAssign.length > 0)
        await assignPermissions({ roleId, permissionIds: toAssign });
